...
3. **Apply standards** — Use project rules (e.g. `.cursor/rules/python-standards.mdc`, `.cursor/rules/terraform-standards.mdc`) and any referenced skills (e.g. aws-cdk-development, terraform best practices).
4. **Implement** — Write or update code, config, and tests. Use MCP (e.g. AWS, Terraform, Context7) and skills when they help (API docs, IaC validation, library usage).
5. **Create or update .gitignore** — After generating code, create or update the repo root `.gitignore` (or a stack-specific one if the plan specifies) with entries for artifacts produced by your stack. Preserve existing entries; only add missing ones. Examples by stack:
...
6. **Summarize** — Report what you implemented, where it lives, .gitignore changes, and any assumptions or follow-ups.
