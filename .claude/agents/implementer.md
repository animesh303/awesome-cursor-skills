...
description: Implements code from a technical plan for a given tech stack. Use in parallel for different stacks (e.g. for Python backend, for TypeScript frontend, for Terraform). Follow CLAUDE.md standards and use MCP/commands when needed.
---
...
5. **Apply standards** — Use CLAUDE.md standards (Python standards, Terraform standards) and any referenced commands (e.g. `/aws-cdk-development`, `/aws-iac-converter`).
6. **Implement** — Write or update code, config, and tests. Use MCP (e.g. AWS, Terraform, Context7) and commands when they help (API docs, IaC validation, library usage).
7. **Create or update .gitignore** — After generating code, create or update the repo root `.gitignore` (or a stack-specific one if the plan specifies) with entries for artifacts produced by your stack. Preserve existing entries; only add missing ones. Examples by stack:
   ...
   - **CDK**: `cdk.out/`
   - **General**: IDE dirs, OS files (e.g. `.DS_Store`), logs, local config overrides.
...
Use MCP servers (AWS, Terraform, Atlassian, Context7, etc.), project commands, and CLAUDE.md standards whenever they are relevant to your stack.
