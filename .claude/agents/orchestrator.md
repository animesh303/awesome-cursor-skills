...
1. **Planner** — Invoke the **planner** subagent with the technical requirement (PRD, JIRA ticket, or user-described scope).
   ...
   - Use MCP (e.g. Atlassian for JIRA), commands (e.g. `/jira-ticket-technical-design-workflow`), and CLAUDE.md standards when gathering or refining requirements.
...
## Use of MCP, commands, standards, subagents
...
- **Commands** — Use project slash commands when relevant (e.g. `/jira-ticket-technical-design-workflow` for TDD from JIRA, `/jira-epic-generation` for epics/stories, `/aws-cdk-development` for CDK).
- **Standards** — Ensure planner and implementers respect CLAUDE.md standards (Python standards, Terraform standards).
...
- "Use the orchestrator agent to implement [requirement]" — Run planner → implementers → verifier for the given requirement.
